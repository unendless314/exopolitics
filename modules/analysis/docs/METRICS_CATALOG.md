...
*   **Formula**: $$\text{Fetch Success Rate} = \frac{\text{Successful Fetch Attempts (fetch\_attempt.outcome = 'success')}}{\text{NULLIF(Total Fetch Attempts, 0)}}$$ where `fetch_attempt.started_at` is within the lookback window.
...
*   **Window Basis**: `source_item_cohort`
*   **Formula**: For each source, compute the proportion for class $x$ (where $x \in \{\text{core}, \text{adjacent}, \text{irrelevant}, \text{unknown}\}$):
    $$\text{Topic Class Proportion}_x = \frac{\text{COUNT(classified items where topic\_class = } x\text{)}}{\text{NULLIF(COUNT(classified items), 0)}}$$
    where `source_item.fetched_at` is within the lookback window.
*   **Data Source**: `classification_result`, `source_item`
...
#### 2.3.2 Average Confidence `[MVP]`
*   **Purpose**: Measure the average confidence score of LLM topic classification.
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Average Confidence} = \text{AVG(classification\_result.classification\_confidence)}$$
    where:
    - The average is computed strictly over items in `classification_result` that have a non-NULL `classification_confidence`.
    - If no classified items exist in the window, or all confidence scores are NULL (zero denominator), emit `NULL`.
*   **Data Source**: `classification_result`, `source_item`
*   **Direct Dimensions**: `source_item_id`
*   **Derived Dimensions**: `source_id` (via joining `source_item` on `source_item_id`)
*   **Update Frequency**: Executed per CLI run.
...
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Curation Approval Rate} = \frac{\text{COUNT(curation\_decision rows where curate\_status = 'approved')}}{\text{NULLIF(Total Curated Items (items with a row in curation\_decision), 0)}}$$ where `source_item.fetched_at` is within the lookback window.
...
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Overall Yield} = \frac{\text{Approved Content Count (items with a row in approved\_content\_record)}}{\text{NULLIF(Total Ingested (items with a row in source\_item), 0)}}$$ where `source_item.fetched_at` is within the lookback window.
...
### 2.8 Stage Conversion Rate `[MVP]`
*   **Purpose**: Evaluate the step-by-step conversion efficiency between consecutive pipeline stages.
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Stage Conversion Rate} = \frac{\text{Current Stage Count}}{\text{NULLIF(Previous Stage Count, 0)}}$$
    where each funnel stage counts distinct source items `COUNT(DISTINCT source_item_id)` belonging to the cohort:
    - **Ingested**: `COUNT(DISTINCT source_item.source_item_id)`
    - **Classified**: `COUNT(DISTINCT classification_result.source_item_id)`
    - **Curated**: `COUNT(DISTINCT curation_decision.source_item_id)`
    - **Approved**: `COUNT(DISTINCT approved_content_record.source_item_id)` joined with `curation_decision` on `source_item_id` where `curation_decision.curate_status = 'approved'` (active approved cohort, excluding withdrawn items).
    - **Translated**: `COUNT(DISTINCT translation_output.source_item_id)` joined with `curation_decision` on `source_item_id` and `approved_content_record` on `source_item_id` and `translation_output` on `parent_content_id` where `curation_decision.curate_status = 'approved'`, `translation_output.translation_status = 'completed'`, and `translation_output.source_fingerprint = approved_content_record.content_fingerprint` (active approved items with completed translations matching the current master-draft fingerprint).
    - **Published**: `COUNT(DISTINCT publish_record.source_item_id)` joined with `curation_decision` on `source_item_id`, `approved_content_record` on `source_item_id`, and `publish_language_status` on `publish_record_id` where `curation_decision.curate_status = 'approved'`, `publish_language_status.publish_status = 'published'`, and `publish_language_status.source_fingerprint = approved_content_record.content_fingerprint` (representing active public artifacts matching the current master-draft fingerprint, excluding withdrawn items, requiring at least one language in a published state).
*   **Data Source**: `source_item`, `classification_result`, `curation_decision`, `approved_content_record`, `translation_output`, `publish_record`, `publish_language_status`
*   **Direct Dimensions**: `source_item_id`
*   **Update Frequency**: Executed per CLI run.

### 2.9 Cumulative Yield `[MVP]`
*   **Purpose**: Track the total pipeline throughput yield from the initial ingest stage to any downstream stage.
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Cumulative Yield} = \frac{\text{Current Stage Count}}{\text{NULLIF(Ingest Volume, 0)}}$$
    where each funnel stage counts distinct source items `COUNT(DISTINCT source_item_id)` belonging to the cohort:
    - **Ingested**: `COUNT(DISTINCT source_item.source_item_id)`
    - **Classified**: `COUNT(DISTINCT classification_result.source_item_id)`
    - **Curated**: `COUNT(DISTINCT curation_decision.source_item_id)`
...
### 2.10 Language Coverage Rate `[MVP]`
*   **Purpose**: Monitor the publication coverage of approved content across target languages.
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Language Coverage Rate}_L = \frac{\text{COUNT(DISTINCT publish\_record.source\_item\_id where publish\_language\_status.language\_code = } L\text{ AND publish\_language\_status.publish\_status = 'published' AND curation\_decision.curate\_status = 'approved' AND publish\_language\_status.source\_fingerprint = approved\_content\_record.content\_fingerprint)}}{\text{NULLIF(Total Active Approved Articles, 0)}}$$
    where:
    - The numerator is the count of distinct articles published in language $L$ that are currently in a published state, belong to the active approved cohort (`curation_decision.curate_status = 'approved'`), and match the current master-draft fingerprint (`publish_language_status.source_fingerprint = approved_content_record.content_fingerprint`).
    - The denominator `Total Active Approved Articles` is defined as:
      `COUNT(DISTINCT approved_content_record.source_item_id)` joined with `curation_decision` on `source_item_id` where `curation_decision.curate_status = 'approved'` (representing the active approved cohort, excluding withdrawn items).
    - Both numerator and denominator are bound to the ingestion cohort (`source_item.fetched_at` within the lookback window).
*   **Data Source**: `approved_content_record`, `curation_decision`, `publish_language_status`, `publish_record`, `source_item`
...
*   **Window Basis**: `source_item_cohort`
*   **Formula**: $$\text{Unique Contribution Rate} = \frac{\text{Curate Approved (Undeduped)}}{\text{NULLIF(Total Ingested, 0)}}$$ where `source_item.fetched_at` is within the lookback window.
...
To diagnose end-to-end bottlenecks, the pipeline is segmented into stage-specific latencies. All metrics in this suite calculate the Average, Median (p50), and 90th percentile (p90) statistics, and are explicitly classified into execution, freshness, or queue delay types. Note: When reported together as part of the E2E latency breakdown in the funnel report (analyze-funnel), stage metrics (except Fetch Execution Latency, which maintains its event_time basis due to the absence of item-level database linkage to fetch attempts) are computed using the source_item_cohort basis.
...
*   **Window Basis**: `event_time`
*   **Formula**: $$\text{Translation Success Rate} = \frac{\text{Successful Translations (translation\_status = 'completed')}}{\text{NULLIF(Total Translation Attempts (translation\_status IN ('completed', 'failed')), 0)}}$$ where `translation_output.updated_at` is within the lookback window.
...
*   **Window Basis**: `source_item_cohort`
*   **Formula**:
    *   **Global Completion Rate (Article-Level)**: $$\text{Translation Completion Rate} = \frac{\text{Active Approved Articles with All Target Translations Completed}}{\text{NULLIF(Total Active Approved Articles, 0)}}$$ where both numerator and denominator are bound to the active approved cohort (`curation_decision.curate_status = 'approved'`) and the ingestion cohort (`source_item.fetched_at` within lookback window).
    *   **Per-Locale Completion Rate (for language } L\text{)}**: $$\text{Translation Completion Rate}_L = \frac{\text{Completed Translations for Language } L\text{ in Active Approved Cohort}}{\text{NULLIF(Total Active Approved Items (where source language } \ne L\text{), 0)}}$$
*   **Data Source**: `approved_content_record`, `curation_decision`, `translation_output`, `source_item`
*   **Direct Dimensions**: `source_item_id`, `language_code`
*   **Derived Dimensions**: `source_id` (via joining `source_item` on `source_item_id`)
*   **Update Frequency**: Executed per CLI run.
*   **SQL Pattern (Global)**:
    ...
    FROM (
        SELECT
            ...
            -- Count distinct target language codes that are completed and match fingerprint
...
*   **Window Basis**: `event_time`
*   **Formula**: $$\text{Translation Stale Rate} = \frac{\text{COUNT(rows where translation\_status = 'stale')}}{\text{NULLIF(COUNT(rows where translation\_status IN ('completed', 'failed', 'stale')), 0)}}$$ where `translation_output.updated_at` is within the lookback window.
*   **Data Source**: `translation_output`
