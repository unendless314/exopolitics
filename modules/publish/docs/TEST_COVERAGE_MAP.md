...
| Node ID | Focus | Protected rule | Unique assertions |
| :--- | :--- | :--- | :--- |
| `TestBootstrapGeneration::test_first_run_with_data_establishes_pointer_and_meta` | — | the first successful run always builds a complete generation and establishes the pointer | exact pointer field values, generation id derived from the run timestamp, `meta.json` aggregate hash set (never `items/*`) matching the on-disk bytes |
| `TestBootstrapGeneration::test_first_run_without_data_establishes_empty_generation_and_pointer` | — | zero-state bootstrap: the pointer and `meta.json` are established even though every aggregate is empty | empty-generation `meta.json` hash set (layout details pinned by `test_zero_state_bootstrap_layout`, section 7) |
