...
| `required_context` | `TEXT` | `NULL` | Publication-critical context the editor or rewrite step must preserve, add, or verify. |
...
**Language Policy**: Under the current system policy, all text fields (`display_title`, `summary_short`, `bullet_1`, `bullet_2`, `bullet_3`) are populated strictly in English (English canonical mother-draft).
...
| `bullet_3` | `TEXT` | `NULL` | Third key bullet summary point capturing the objective implication of the development (NULL for `publish_link` items). |
| `created_at` | `TEXT` | `NOT NULL` | UTC ISO-8601 timestamp. |
| `updated_at` | `TEXT` | `NOT NULL` | UTC ISO-8601 timestamp. |
