...
* **Queue Selection:** The pending queue query must filter on `t.text_processing_status != 'failed'` and `(t.text_processing_reason IS NULL OR t.text_processing_reason != 'post_cleanup_empty')` (jointly with `s.ingest_status = 'ingested'`). Only `failed` items and `post_cleanup_empty` outcomes are excluded during item selection; all other items, including low-context ones, enter classification processing.
