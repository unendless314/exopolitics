...
The module fetches pending `source_item` records (those with status `'ingested'` and no existing classification result), evaluates them using feed metadata, and writes the output classification parameters (such as `topic_class`, `classification_reason`, `classification_confidence`, and `edit_candidate` flags) to the canonical database.
...
* Detect low-context items and classify them as `unknown` without invoking the LLM.
