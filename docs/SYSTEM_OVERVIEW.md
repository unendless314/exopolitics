...
- pending item selection (excluding only items where `text_processing_status` is `failed` or `text_processing_reason` is `post_cleanup_empty`; all other items, including normal `low_context` outcomes, enter classify)
